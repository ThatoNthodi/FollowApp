...
def login(form_data: OAuth2PasswordRequestForm = Depends(), db: Session = Depends(get_db)):
    ...
    # A single login form serves both clinicians and patients - we check
    # both tables and route based on wherever the email/password matches.
    ...
    if patient and patient.hashed_password and verify_password(
...
def register_patient(signup: schemas.PatientSignup, db: Session = Depends(get_db)):
    ...
    existing_with_password = (
        ...
        .filter(models.Patient.email == signup.email)
        .filter(models.Patient.hashed_password.isnot(None))
        .first()
    )
    if existing_with_password:
        raise HTTPException(status_code=400, detail="A patient account with this email already exists")

    # Find any unclaimed record (no password yet) matching by email or phone,
    # and attach this new login to it instead of creating a duplicate patient.
    linked_patient = (
        db.query(models.Patient)
        .filter(models.Patient.hashed_password.is_(None))
        .filter(
            (models.Patient.email == signup.email)
            | (models.Patient.phone_number == signup.phone_number)
        )
        .first()
    )

    if linked_patient:
        linked_patient.email = signup.email
        linked_patient.full_name = signup.full_name
...
def get_me(token: str = Depends(oauth2_scheme), db: Session = Depends(get_db)):
    ...
    if role == "clinician":
        ...
        if not clinician:
            raise credentials_error
        return schemas.MeResponse(role="clinician", clinician=clinician)
    elif role == "patient":
        patient = db.query(models.Patient).filter(models.Patient.id == user_id).first()
        if not patient:
            raise credentials_error
        return schemas.MeResponse(role="patient", patient=patient)
    raise credentials_error
...
def get_patient_portal(patient_id: str, db: Session = Depends(get_db)):
    ...
    login - no clinical data can be modified through this endpoint. Kept
    alongside /me/portal as a no-login-required share option."""
    return _build_portal_view(patient_id, db)


def _build_portal_view(patient_id: str, db: Session) -> schemas.PatientPortalView:
